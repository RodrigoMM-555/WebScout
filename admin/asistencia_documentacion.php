...
if (!empty($secciones)) {
    ...
    $stmtEdu = $conexion->prepare("SELECT id, nombre, apellidos, seccion FROM educandos WHERE seccion IN ($placeholders) ORDER BY FIELD(seccion,'colonia','manada','tropa','posta','rutas')");
...
// Pintar tabla
echo "<div class='asi'><h1>Asistencia y documentación: " . htmlspecialchars($tituloAviso) . "</h1>
        <a href='index.php?tabla=avisos'>Volver a avisos</a></div>";

foreach (['si','pendiente','no'] as $key) {
    $label = $key==='si' ? 'Asisten' : ($key==='no' ? 'No asisten' : 'Pendientes');
    echo "<h2>$label</h2>";
    if (empty($grupos[$key])) { echo "<p>No hay registros</p>"; continue; }

    echo "<table class='asistencia'>
            <tr><th>Niñ@</th><th>Sección</th>";
    if ($key==='si') echo "<th>Entregado</th>";
    echo "</tr>";
    ...
    foreach ($grupos[$key] as $edu) {
        ...
        if ($key == 'si'|| $key == 'pendiente') {
            echo "<tr class='seccion-{$edu['seccion']}'><td>" . htmlspecialchars($nombre) . "</td><td>$seccion</td>";
        } else {
            echo "<tr><td>" . htmlspecialchars($nombre) . "</td><td>$seccion</td>";
        }
        ...
        if ($key==='si') {
            ...
            if (is_dir($ruta)) {
                ...
                foreach ($archivos as $f) if (strpos($f, $prefijo) === 0) { $entregado = true; break; }
            }
